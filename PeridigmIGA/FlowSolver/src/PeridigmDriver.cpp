...
int main(int argc, char *argv[]) {
  ...
  if(mpi_id == 0){
    cout << "\n----IGA Flow Solver for TEMPI Coupling--" << endl ;
...
manager.sync();
   ...
   manager.sync();
    ...
    for (it=0; it<user.max_its ; it++) {
      ...
      if(!info.isTask){
       ...
       /*Need to transfer kinematic info at this point*/
  ...
  /* We Need Forces from Peridigm at this point*/
...
/*Need to handoff kinematics from IGA to Peridigm at this point*/
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  par->stepNumber++;
  //Need to update solution state at this point in peridigm as well 
  
  if (par->stepNumber % par->FreqResults == 0) {
       char filename[256];
       sprintf(filename,"velS%d.dat",par->stepNumber);
       ierr = IGAWriteVec(user.iga,user.V1,filename);CHKERRQ(ierr);
       //Write result to an ExodusII file at FreqResult Interval in Peridigm
       
   }

   // Dump residual At restart interval
   if (par->stepNumber %  RestartStep == 0) {
       //Write enough to restart peridigm
       char filename[256];
