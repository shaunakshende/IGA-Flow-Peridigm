...
public:
  ...
  & flag & flag0 & inertia & residual & bodyForce & internalForce & referenceNodalVolume & alphaNodalDensity & alphaNodalVolume & ID & ID_PD & material & referenceDensity
  & referenceCoord & currentCoord;
  } // end serialize
}; // end class fieldData
...
PetscErrorCode FormInitialCondition(IGA iga,PetscReal t,Vec U,AppCtx *user)
{
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      ...
      //RDX Charge
      PetscReal Standoff = 0.152;
      if(r<=(0.0049+h+0.000001) && z<=(0.00126+Standoff+0.0047+h+0.000001) && z>=(0.00126+Standoff-h-0.000001)){
        u[k][j][i].ux   =  0.0;
        u[k][j][i].uz   =  0.0;
        u[k][j][i].uy   =  0.0;
        u[k][j][i].rho  =  1770.0;
...
PetscErrorCode ComputeCurrentExplosiveVolume(AppCtx *user, PARAMETERS *par, ParticleManager &manager)
{
  ...
  for(auto it=its.first; it != its.second; ++it){
    ...
    if(fd.material==2){
      //fd.nodalDensity*=1.225/1000.0;
      fd.nodalPressure = P0*fd.nodalDensity/1.225;
    }
...
PetscErrorCode ResidualAIR(IGAProbe prb,
                          ...
                          AppCtx *user)
{
  /// AIR residual for Thermally decoupled problems (but not normalized air) ///
    PetscErrorCode ierr;
    PetscInt  i,j,l,m;
    PetscInt  dof   = prb->dof;
    PetscInt  dim   = prb->dim;
    PetscReal mu     = 10.0;//1.81e-5;
                                     ...
                                     Vec vecRes, ParticleManager &manager)
{
  ...
  for(auto it=its.first; it != its.second; ++it){
    ...
    // }
    // else{
    //   PetscPrintf(PETSC_COMM_WORLD, "Damage Cap hit\n");
    //   fd.penaltyParameter = (1.0 - par->thresholdDamageForPenalty) * fd.referencePenaltyParameterInternal;
    // }


    for(i=0; i<dim; i++){
      //PetscPrintf(PETSC_COMM_WORLD, "interpolated = %e physical = %e \n", fd.interpolatedVelocity[i], fd.totalPhysicalVelocity[i]);
    ...
    for (a=0;a<user->nen;a++){
      ...
      for(i=0; i<dim; i++){
        value = fd.penaltyParameter * (prb->shape[0][a]*velDiff[i]) * fd.referenceNodalVolume;
...
int main(int argc, char *argv[]) {
  ...
  materialParams.sublist("My Shell Material").set("Number Of Shell Layers", 3);
  ...
  solverParams->sublist("Verlet");
  solverParams->sublist("Verlet").set("Fixed dt", par->timeStep);
  // Specify safety factor if using Verlet solver on PD side
  //solverParams->sublist("Verlet").set("Safety Factor", 0.1);
  solverParams->set("Initial Time", user.PDInitialTime);
  solverParams->set("Final Time", user.OutputRestart);
  solverParams->set("Verbose", false);
  ...
  outputParams.set("Initial Output Step", user.StepRestart);
...
manager.sync();
    ...
    ParticleInfo info = get(info_property,v);
     if(!info.isTask){
       if (fd.Boundary == 0){
         ...
         fd.totalPhysicalDisplacement[0] += par->timeStep*par->timeStep/2.0*((1-2.0*user.Beta)*fd.totalPhysicalAccelerationOldStep[0]+2.0*user.Beta*fd.totalPhysicalAcceleration[0]);

         fd.totalPhysicalDisplacement[1] = fd.totalPhysicalDisplacementOldStep[1] + par->timeStep*fd.totalPhysicalVelocityOldStep[1];
         fd.totalPhysicalDisplacement[1] += par->timeStep*par->timeStep/2.0*((1-2.0*user.Beta)*fd.totalPhysicalAccelerationOldStep[1]+2.0*user.Beta*fd.totalPhysicalAcceleration[1]);

         fd.totalPhysicalDisplacement[2] = fd.totalPhysicalDisplacementOldStep[2] + par->timeStep*fd.totalPhysicalVelocityOldStep[2];
         fd.totalPhysicalDisplacement[2] += par->timeStep*par->timeStep/2.0*((1-2.0*user.Beta)*fd.totalPhysicalAccelerationOldStep[2]+2.0*user.Beta*fd.totalPhysicalAcceleration[2]);
  ...
  //Teuchos::RCP<Epetra_Vector> damage = peridigm->getDamage();
  MPI_Barrier(PETSC_COMM_WORLD);
  ...
  //Debugging
  // ierr = MPI_Allreduce(&testk,&reducedk, 1,MPI_INT,MPI_SUM,PETSC_COMM_WORLD);CHKERRQ(ierr);
  ...
  for(m=0;m<nodesZ;m++) {
      for(l=0;l<nodesY;l++) {
          for(k=0;k<nodesX;k++) {
           ...
           //Simply supported
           if(XX<=h_f*0.9999 || (XX-0.305) >= h_f*0.9999 || YY<=h_f*0.9999 || (YY-0.305) >= h_f*0.9999 ) {
            PetscInt index = (m*gnodesX*gnodesY + l*gnodesX+ k)*dof+3;
            PetscInt index_array[5]={0};
            index_array[0]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof;
            index_array[1]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof+1;
  ...
  peridigm->updateState();
   ...
   if (par->stepNumber % par->FreqResults == 0) {
    ...
    if (par->stepNumber %  RestartStep == 0) {
        ...
        if (par->stepNumber % RestartStep == user.FreqRestarts-1){
