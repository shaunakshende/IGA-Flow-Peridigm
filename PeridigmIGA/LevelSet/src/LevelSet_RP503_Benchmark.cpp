...
typedef struct {
  PetscScalar rho, ux, uy, uz, temp, phi;
} Field;
...
*out = 0.0;
if(sqrt(phi*phi)<h){
  *out = 0.5*(1+phi/h+1/M_PI*sin(phi*M_PI/h));
}
if(phi>=h){
  *out = 1.0;
}
...
PetscReal H;
H = 0.0;
if(sqrt(phi*phi)<epsilon){
  H = 0.5/epsilon*(1.0+cos(phi*M_PI/epsilon));
}
PetscReal out = 2.0*H-1.0;
...
PetscReal P0     = 100000.0;
PetscReal A      = 5.484e11;
PetscReal B      = 0.09375e11;
PetscReal C      = 1.62e9;
PetscReal R1     = 4.94;
PetscReal R2     = 1.21;
PetscReal omega  = 0.28;
PetscReal E0     = 4.2814e6;
...
PetscErrorCode FormInitialCondition(IGA iga,PetscReal t,Vec U,AppCtx *user)
{
  ...
  PetscReal h = (1.0/20.0)/40.0;//(user->Lx)/user->iga->elem_sizes[0];
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      ...
      u[k][j][i].phi  =  sqrt(1); //Distance to cylinder surface
                          ...
                          AppCtx *user)
{
                         ...
                         void *ctx)
 {
  ...
  A0[4][0] = Cv*temp;
  A0[4][4] = dens*Cv;
  A0[4][5] = dAlphadPhiTilde*energy_1;
  A0[5][5] = 1.0;
  ...
  A0inv[4][0] =  -temp/dens;
  A0inv[4][4] =  1.0/(Cv*dens);
  A0inv[4][5] = -dAlphadPhiTilde*temp/(dens*e);
  A0inv[5][5] = 1.0;
  ...
  A1_c[4][1] = dens*Cv*temp;
  A1_c[4][4] = dens*ux*Cv;
  A1_c[4][5] = ux*dAlphadPhiTilde;
  A1_c[5][5] = ux;


  // A_1^p
  A1_p[1][0] = fprime;
  A1_p[1][5] = dAlphadPhiTilde*P_1;
  ...
  A2_c[4][2] = dens*Cv*temp;
  A2_c[4][4] = dens*uy*Cv;
  A2_c[4][5] = uy*dAlphadPhiTilde;
  A2_c[5][5] = uy;

  // A_2^p
  A2_p[2][0] = fprime;
  A2_p[2][5] = dAlphadPhiTilde*P_1;
  ...
  A3_c[4][3] = Cv*temp*dens;
  A3_c[4][4] = dens*Cv*uz;
  A3_c[4][5] = uz*dAlphadPhiTilde;
  A3_c[5][5] = uz;

  //A_3^p
  A3_p[3][0] = fprime;
  A3_p[3][5] = dAlphadPhiTilde*P_1;
...
//########## Main Function #############//
//TODO: clean up pointRank routine, now functioning with stretched meshes
// Remove ElementInfo aspect of setup (delete -- getting knots from iga directly)
//add break; to igalocateelement routine for efficiency
// clean up main using new IGA-Flow-PD code structure
// Add parallel restart functionality for larger problems -- not super necessary for shell
#undef __FUNCT__
...
int main(int argc, char *argv[]) {
  ...
  user.spacing  = 0.0000;
  user.Lx       = 0.35+user.spacing;
  user.Ly       = 0.35+user.spacing;
  user.Lz       = 0.35+user.spacing;
