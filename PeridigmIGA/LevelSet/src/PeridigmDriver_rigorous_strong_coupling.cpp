...
public:
  ...
  template<typename Archiver>
  void serialize(Archiver& ar, const unsigned int version){
    ar & nodalVolume & nodalDensity & nodalDensityInitial & nodalPressure & nodalVolumeInitial & Boundary & totalPhysicalDisplacement & totalPhysicalVelocity & totalStrain & totalStress & totalStrain0 & totalStress0
...
public:
  ...
  int pointRank(double *x, AppCtx *user){
    ...
    if (elemX == user->iga->elem_sizes[0]){
      elemX = user->iga->elem_sizes[0]-1;
    }
    if(elemY == user->iga->elem_sizes[1]){
      elemY = user->iga->elem_sizes[1]-1;
    }
    if(elemZ == user->iga->elem_sizes[2]){
...
PetscErrorCode ParticleDistribute(PARAMETERS *par, AppCtx *user, ParticleManager &manager)
{
  ...
  if(fd.material==0 && fd.ID>user->numFluidNodes && !info.isTask){
    ...
    //Debugging
    // if(rank==0){
    // PetscPrintf(PETSC_COMM_WORLD, "fd.ID_PD = %d\n", fd.ID_PD);
    // }
    if(fd.ID_PD<0){
...
//  ierr = PetscViewerDestroy(&viewer);
  ...
  Stress.open(fname, ios::in | ios::binary);
    ...
    fd.totalPhysicalDisplacement[2] = info.currentCoord[2] - fd.totalPhysicalDisplacement[2];
     ...
     //PetscPrintf(PETSC_COMM_WORLD," Read Nodal Vol %e \n", fd.nodalVolume);
     if (fd.material == 1){
...
PetscErrorCode FormInitialCondition(IGA iga,PetscReal t,Vec U,AppCtx *user)
{
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      ...
      if(r<=0.0049+0.00305*2 && z<=(0.030+0.00126+Standoff+0.0047+0.00305*2)&& z>=(0.030+0.00126+Standoff-0.00305*2)){
        u[k][j][i].ux   =  0.0;
        u[k][j][i].uz   =  0.0;
        u[k][j][i].uy   =  0.0;
        u[k][j][i].rho  =  1770.0;
      ...
      //Setting conditions in the air chamber
       // if(z<=(5*h+1.0*h)){
       //   u[k][j][i].ux   =  0.0;
       //   u[k][j][i].uz   =  0.0;
       //   u[k][j][i].uy   =  0.0;
...
PetscErrorCode ComputeCurrentExplosiveVolume(AppCtx *user, PARAMETERS *par, ParticleManager &manager)
{
  ...
  for(auto it=its.first; it != its.second; ++it){
    ...
    if(fd.material==2){
      fd.nodalPressure = dens*287.05*T;
    }
...
// #undef  __FUNCT__
// #define __FUNCT__ "ResidualAIR"
// PetscErrorCode ResidualAIR(IGAProbe prb,
//                           IGAProbe prb_t,
//                           IGAElement ele,
//                           PetscReal dt,
//                           PetscReal t,
...
//    A1_c[0][0] = ux;
//    A1_c[0][1] = dens;
//    A1_c[0][4] = -ux*dens/temp;
//
//    A1_c[1][0] = ux*ux;
//    A1_c[1][1] = 2.0*dens*ux;
//    A1_c[1][4] = -ux*ux*dens/temp;
//
//    A1_c[2][0] = ux*uy;
//    A1_c[2][1] = dens*uy;
//    A1_c[2][2] = dens*ux;
//    A1_c[2][4] = -ux*uy*dens/temp;
//
//    A1_c[3][0] = ux*uz;
//    A1_c[3][1] = dens*uz;
//    A1_c[3][3] = dens*ux;
//    A1_c[3][4] = -ux*uz*dens/temp;
//
//    A1_c[4][0] = Cv*temp*ux;
...
//    A2_c[0][0] = uy;
//    A2_c[0][2] = dens;
//    A2_c[0][4] = -uy*dens/temp;
//    A2_c[1][0] = ux*uy;
//    A2_c[1][1] = dens*uy;
//    A2_c[1][2] = dens*ux;
//    A2_c[1][4] = -ux*uy*dens/temp;
//    A2_c[2][0] = uy*uy;
//    A2_c[2][2] = 2.0*dens*uy;
//    A2_c[2][4] = -uy*uy*dens/temp;
//    A2_c[3][0] = uy*uz;
//    A2_c[3][2] = dens*uz;
//    A2_c[3][3] = dens*uy;
//    A2_c[3][4] = -uy*uz*dens/temp;
//    A2_c[4][0] = uy*Cv*temp;
...
//    A3_c[0][0] = uz;
//    A3_c[0][3] = dens;
//    A3_c[0][4] = -uz*dens/temp;
//    A3_c[1][0] = ux*uz;
//    A3_c[1][1] = dens*uz;
//    A3_c[1][3] = dens*ux;
//    A3_c[1][4] = -uz*ux*dens/temp;
//    A3_c[2][0] = uy*uz;
//    A3_c[2][2] = dens*uz;
//    A3_c[2][3] = dens*uy;
//    A3_c[2][4] = -uz*uy*dens/temp;
//    A3_c[3][0] = uz*uz;
//    A3_c[3][3] = 2.0*dens*uz;
//    A3_c[3][4] = -uz*uz*dens/temp;
//    A3_c[4][0] = Cv*temp*uz;
...
// fluid domain
  ...
  if((!info.isTask) && (fd.Inside == 1)){// && (fd.material!=0)){
      ...
      if(fd.material==2){
       // Density Primitive A0_air:
        Cv    = 718.0;
        A01[0][0] = 1.0;
        A01[0][4] = -dens/temp;
        A01[1][0] = ux;
        A01[1][1] = dens;
        A01[1][4] = -ux*dens/temp;
        A01[2][0] = uy;
        A01[2][2] = dens;
        A01[2][4] = -uy*dens/temp;
        A01[3][0] = uz;
        A01[3][3] = dens;
        A01[3][4] = -uz*dens/temp;
        A01[4][0] = Cv*temp;
      }
                            ...
                            Vec vecF, ParticleManager &manager)
{
  ...
  if(IGALocateElement_1(user->iga,pt,ele1)){
    ...
    }else{if(fd.material == 2){
    ierr = ResidualAIR(prb, prb_t, ele1, dt, t, Kvec1, user);CHKERRQ(ierr);
    }}
...
int main(int argc, char *argv[]) {
  ...
  if(mpi_id == 0){
    cout << "\n--Peridigm-PD Shells + ImmersedIGA Compressible flow--" << endl ;
    cout << "Petsc Version\n" << version<< endl ;
    cout << "Using Boost "
  ...
  user.spacing  = 0.0000000000000001;
  user.Lx       = 0.5+user.spacing;
  user.Ly       = 0.5+user.spacing;
  user.Lz       = 0.5+1.0*user.spacing;
  ...
  if (par->stepNumber == 0){
    PetscInt checkSize;
    int checkSize2;
    PetscMPIInt checksize3;
    PetscReal realCheck;
    double  doubleCheck;
    PetscScalar scalarCheck;

    PetscPrintf(PETSC_COMM_WORLD, "Configured with Integer sizes %lu %lu and %lu\n", sizeof(checkSize)*8, sizeof(checkSize2)*8, sizeof(checksize3)*3);
    PetscPrintf(PETSC_COMM_WORLD, "Configured with Real sizes %lu %lu and %lu\n", sizeof(realCheck)*8, sizeof(doubleCheck)*8, sizeof(scalarCheck)*3);
  ...
  materialParams.sublist("My Shell Material").set("Density", par->density);
  materialParams.sublist("My Shell Material").set("Young's Modulus", par->youngModulus);
  materialParams.sublist("My Shell Material").set("Poisson's Ratio", par->poissonRatio);
  materialParams.sublist("My Shell Material").set("Shell Thickness", user.thickness);
  ...
  blockParams.set("Horizon", 3.0*0.305/149.0);
  ...
  BlockParams.sublist("My Block").set("Horizon", 3.0*0.305/149.0);
  //
  ...
  outputParams.set("Output Frequency", par->FreqResults);
  ...
  double VEL[num_PD_nodes*3];
  double DISP[num_PD_nodes*3];
  double FORCE[num_PD_nodes*3];
  double COORD[num_PD_nodes*3];
...
manager.sync();
  ...
  if(par->stepNumber==0){
   ...
   manager.sync();
    ...
    for (it=0; it<user.max_its ; it++) {
       ...
       PetscPrintf(PETSC_COMM_WORLD,":::::::::::::::::::::::::::::::::\n");
       user.it = it;
       //Debugging - rank iteration timer
       //double t1 = MPI_Wtime();
      ...
      manager.sync();
       ...
       manager.sync();
       ierr = ComputeDeformationGradient(par,Alpha_f,manager);CHKERRQ(ierr);
       manager.sync();
  ...
  num_PD_nodes_onRank = (int)force->MyLength()/3.0;
  int GID[num_PD_nodes_onRank];
  manager.sync();

  // Obtain Global node ID's for points on the current rank in order (numbered 0-N-1 instead of 1-N)!!
  ...
  for(i = 0 ; i < num_PD_nodes_onRank; i++){
    for(j = 0 ; j < 3 ; j++){
      ...
      user.COORD[GID[i]*3+j] = (*currentPosition)[i*3+j];
    }
  }
  ...
  BGL_FORALL_VERTICES(v,manager.graph,Graph){
    ...
    if(!info.isTask && fd.material == 0){
      ...
      for(j = 0 ; j < 3 ; j++){
        fd.internalForce[j]  = FORCE[fd.ID_PD*3+j];
        info.currentCoord[j] = COORD[fd.ID_PD*3+j];
        fd.bodyForce[j] = 0.0;
      }
    }
    put(fd_property, v, fd);
...
////////////////////////////////////////////////////////////////////
  ...
  manager.sync();
  ierr = computeSolidResidualWeak(par, &user, ResS, manager);CHKERRQ(ierr);
  manager.sync();
  ///////////////////////////////////////////////////////////////

  ///// Mass Matricies /////
  ierr = lumpedMass(par, &user, Mass, Alpha_m, manager);CHKERRQ(ierr);
  manager.sync();
  ...
  ierr = MatDiagonalSet(A0,Mass,ADD_VALUES);CHKERRQ(ierr);
  manager.sync();
  //////////////////////////////////////
  ...
  ierr = VecZeroEntries(Res);CHKERRQ(ierr);
  ierr = VecAYPX(Res,1.0,ResF);CHKERRQ(ierr);
  ...
  for(m=0;m<nodesZ;m++) {
      for(l=0;l<nodesY;l++) {
          for(k=0;k<nodesX;k++) {
           ...
           if(ZZ <= 5*h_x-0.0001){
           if(abs(XX)<=h_x || abs(0.305-XX)<=h_x){
             PetscInt index = (m*gnodesX*gnodesY + l*gnodesX+ k)*dof+1;
             PetscInt index_array[5]={0};
             index_array[0]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof;
             index_array[1]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof+1;
            ...
            if(abs(YY)<=h_x || abs(0.305-YY)<=h_x){
             PetscInt index = (m*gnodesX*gnodesY + l*gnodesX+ k)*dof+2;
             PetscInt index_array[5]={0};
             index_array[0]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof;
             index_array[1]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof+1;
           ...
           if(abs(ZZ-5*h_x)<h_x){
            ...
            if(abs(XX)<=h_x || abs(0.305-XX)<=h_x || abs(YY)<=h_x || abs(0.305-YY)<=h_x) {
             PetscInt index = (m*gnodesX*gnodesY + l*gnodesX+ k)*dof+3;
             PetscInt index_array[5]={0};
             index_array[0]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof;
             index_array[1]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof+1;
           ...
           //Fixing temperature everywhere for stability (Hard to avoid with water-JWL):
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //peridigm->updateState();
   ...
   if (par->stepNumber % par->FreqResults == 0) {
        ...
        peridigm->writePeridigmSubModel(par->stepNumber);
    }
