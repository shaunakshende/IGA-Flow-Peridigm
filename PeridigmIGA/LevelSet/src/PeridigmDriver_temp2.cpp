...
// PetscErrorCode Alpha(AppCtx *user, PetscReal phi, PetscReal* out, PetscReal h){
// // Regularized Heavyside functon for Level set formulation (signed difference)
// PetscErrorCode ierr;
// PetscFunctionBegin;
// //PetscReal h = user->Lx/user->iga->elem_sizes[0];
// *out = 0.0;
// if(sqrt(phi*phi)<h){
//   *out = 0.5*(1.0+phi/h+1/M_PI*sin(phi*M_PI/h));
// }
// if(phi>=h){
//   *out = 1.0;
// }
...
#undef  __FUNCT__
#define __FUNCT__ "alpha"
PetscReal alpha(AppCtx *user, PetscReal phi){
PetscFunctionBegin;
PetscReal out = 0.5*(tanh(phi/(user->Lx/user->iga->elem_sizes[0]))/1.0e0+1.0);
//PetscPrintf(PETSC_COMM_WORLD, "%e %e\n", phi/(user->Lx/user->iga->elem_sizes[0]), tanh(phi/(user->Lx/user->iga->elem_sizes[0])));
PetscFunctionReturn(out);
}


#undef  __FUNCT__
#define __FUNCT__ "dAlphadPhi"
PetscErrorCode dAlphadPhi(AppCtx *user, PetscReal phi, PetscReal* out, PetscReal h){
// Derivative Regularized Heavyside functon for Level set formulation (signed difference)
PetscErrorCode ierr = 0;
PetscFunctionBegin;
//PetscReal h = user->Lx/user->iga->elem_sizes[0];
*out = 0.0;
if(sqrt(phi*phi)<h){
  *out = 0.5/h*(1.0+cos(phi*M_PI/h));
}
PetscFunctionReturn(ierr);
}

#undef  __FUNCT__
#define __FUNCT__ "dAlphatildedPhi"
PetscReal dAlphatildedPhi(AppCtx *user, PetscReal alphatilde, PetscReal alpha_sum, PetscReal dAlphadPhi){
// Derivative Regularized Heavyside functon for Level set formulation (signed difference)

PetscFunctionBegin;
PetscReal out = (1.0/alpha_sum)*dAlphadPhi;//*(1.0+alphatilde);
...
PetscFunctionBegin;
PetscReal H;
//H =  0.5*(tanh(phi/epsilon)/1.0e0+1.0);
H = tanh(phi/epsilon);
PetscFunctionReturn(H);
}
                                    ...
                                    PARAMETERS *par)
{
  ...
  //Build the LS function based on what we know about the geometry of the explosive at timestep 0:
  if(par->stepNumber==0){
    for(i=info.xs;i<info.xs+info.xm;i++){
      for(j=info.ys;j<info.ys+info.ym;j++){
        for(k=info.zs;k<info.zs+info.zm;k++){
          ...
          if(z > height && r > R){
            phi = -sqrt((R-r)*(R-r) + (height-z)*(height-z));
          }
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      for(k=info.zs;k<info.zs+info.zm;k++){
        ...
        if(alpha_1 > 0.0 && alpha_1 < 1.0-0.00001){
          //Will have to change for a stretched grid to localwidth instead
          u_LS[k][j][i].phi = (user->Lx/user->iga->elem_sizes[0])*atanh(1.0e0*(2.0*alpha_1-1.0));
         }
        }
      }
    }
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      for(k=info.zs;k<info.zs+info.zm;k++){
        ...
        if(u[k][j][i].alpha > 0.0 && u[k][j][i].alpha < 1.0-0.00001){
          u[k][j][i].alpha = 0.5*(tanh(phi/h)/1.0e0+1.0);
        }
      }
    }
  }
...
#undef  __FUNCT__
#define __FUNCT__ "IGALocateElement_1_temp"
PetscBool IGALocateElement_1_temp(IGA iga,PetscReal *pnt,IGAElement element, AppCtx *user, ParticleManager &manager)
{
  PetscFunctionBegin;
  ...
  for(i=0;i<dim;i++){
    ...
    for(j=0;j<m;j++){
      if(U[j+1]-U[j]>1.0e-13) e += 1;
      if((pnt[i] >= U[j] && pnt[i] < U[j+1]) || sqrt((pnt[i]-1.0)*(pnt[i]-1.0)) < 1.0e-13){ID[i] = e;}
    }
    /* reject if the element is not in this partition */
    if(ID[i] < iga->elem_start[i] || ID[i] >= iga->elem_start[i]+iga->elem_width[i]){
    PetscPrintf(PETSC_COMM_SELF, "Error: Particle on domain but not located by IGALocateElement! This Probably means that the rank of the point \n and the rank of the Subdomain assigned to rank are not consistent when this function is being called! Check pointRank. Exiting...\n");
...
PetscErrorCode FormInitialCondition(IGA iga,PetscReal t,Vec U,AppCtx *user)
{
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      ...
      u[k][j][i].alpha = 0.5*tanh(phi/h)+0.5;
      u[k][j][i].rho = (1.0-u[k][j][i].alpha)*1000.0 + u[k][j][i].alpha*1770.0;
    }
  }
}
  ierr = DMDAVecRestoreArray(da,U,&u);CHKERRQ(ierr);
...
PetscErrorCode probePressureSignals(AppCtx *user, PARAMETERS *par)
{
  PetscFunctionBegin;
  //PetscMPIInt rank;
  //MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
...
F3[3] = P;
  ...
  tau_ls = sqrt(Res[5]*Res[5])/
  sqrt(hu[3]+1e-15);

  if(DC*tau_c > k_cap){tau_c = k_cap;}else{tau_c = DC*tau_c;}
  if(tau_ls > k_cap){tau_ls = k_cap;}else{tau_ls = tau_ls;}
  tau_m = tau_c;
  tau_t = tau_c;
  ...
  for (a=0; a<nen; a++) {
    ...
    // for (i=0;i<dof;i++){
    //   for (j=0;j<dof;j++){
    //       R[a][i] += - A1_p[i][j]*N1[a][0]*u[j] - A2_p[i][j]*N1[a][1]*u[j] - A3_p[i][j]*N1[a][2]*u[j];
    //   }
    // }
...
//Stabilization terms SUPG & DC
                         ...
                         void *ctx)
 {
     ...
     PetscScalar alpha_1 =  0.5*(tanh(phi/(user->Lx/user->iga->elem_sizes[0]))+1.0);
     ...
     for (int i=0;i<dim;i++){
       ...
       PetscReal localWidth = user->Lx/user->iga->elem_sizes[0];//0.5*sqrt(grad_u[0][0]*grad_u[0][0]+grad_u[0][1]*grad_u[0][1]+grad_u[0][2]*grad_u[0][2])/sqrt(gradPhiGgradPhi);
       PetscReal phi_reference = (localWidth)*atanh(1.0e0*(2.0*alpha_1-1.0));
     ...
     for (a=0; a<nen; a++) {
       ...
       if(!pnt->atboundary){
         ...
         R[a][0] += (N0[a] + Tau_phi*(convVel[0]*N1[a][0]+convVel[1]*N1[a][1]+convVel[2]*N1[a][2]))*(u_t[0]+S*(normGradPhi-1.0));}
         //(u_t[0]+convVel[0]*grad_u[0][0]+convVel[1]*grad_u[0][1]+convVel[2]*grad_u[0][2]-S);}

         //Penalty Term for large density difference or pressure driven expansion flows
         //R[a][0] += N0[a]*2.0*(phi-phi_reference);
                         ...
                         AppCtx *user)
 {
     ...
     PetscReal   h = user->Lx/user->iga->elem_sizes[0];
     PetscScalar alpha_1_reference = 0.5*(tanh(phi0/h)/1.0e0+1.0);
     ...
     for (a=0; a<nen; a++) {
       R[a][0] = 0.0;
       if(!pnt->atboundary){
         R[a][0] += N0[a]*(alpha(user, phi_tilde) - alpha_1_reference);
                       ...
                       PARAMETERS *par)
{
  ...
  PetscReal localWidth    = 0.5*sqrt(grad_u[0][0]*grad_u[0][0]+grad_u[0][1]*grad_u[0][1]+grad_u[0][2]*grad_u[0][2])/sqrt(gradPhiGgradPhi);
  ...
  for(int a = 0 ; a<nen ; a++){
    //for(int b = 0 ; b<nen ; b++){
     KK[a][0][a][0] += (N0[a]+(convVel[0]*N1[a][0]+convVel[1]*N1[a][1]+convVel[2]*N1[a][2])*Tau_phi)*shift;
  //}
}
...
//  PetscPrintf(PETSC_COMM_WORLD, "%e\n",phi_prime);
  ...
  for(int a = 0 ; a<nen ; a++){
  //  for(int b = 0 ; b<nen ; b++){
     KK[a][0][a][0] += N0[a]*0.5*(1.0-tanh(phi_tilde/h)*tanh(phi_tilde/h))/h;
                                       ...
                                       PARAMETERS *par)
{
  ...
  while (IGANextElement(iga,element) && IGANextElement(iga_main, element_main)) {
    ...
    ierr = IGAElementEndPoint(element,&point);CHKERRQ(ierr);
    ierr = IGAElementEndPoint(element_main,&point_main);CHKERRQ(ierr);
    ierr = IGAElementFixJacobian(element,J);CHKERRQ(ierr);
    ierr = IGAElementAssembleMat(element,J,matJ);CHKERRQ(ierr);
  }

  ierr = IGAEndElement(iga,&element);CHKERRQ(ierr);
  ierr = IGAEndElement(iga_main,&element_main);CHKERRQ(ierr);
...
//##################################################################################################
 ...
 ierr = InitializeLSC(iga_LSC, user.V0_LSC);CHKERRQ(ierr);
  ...
  for(int massBalance = 0 ; massBalance < 30 ; massBalance++){
   ...
   PetscInt dof_LSC = iga_LSC->dof;
   PetscScalar bc_phi[1]={0.0};
   bc_phi[0]=1.0;
   PetscInt nodesX  = iga_LSC->geom_lwidth[0], nodesY  = iga_LSC->geom_lwidth[1], nodesZ  = iga_LSC->geom_lwidth[2];
   PetscInt gnodesX = iga_LSC->geom_gwidth[0], gnodesY = iga_LSC->geom_gwidth[1];
   for(m=0;m<nodesZ;m++) {
     for(l=0;l<nodesY;l++) {
       for(k=0;k<nodesX;k++) {
        if(   (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim]   <= 0.00001+user.spacing/2.0) || (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim]   >= user.Lx-0.00001-user.spacing/2.0)
           || (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim+1] <= 0.00001+user.spacing/2.0) || (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim+1] >= user.Ly-0.00001-user.spacing/2.0)
           || (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim+2] <= 0.00001+user.spacing/2.0) || (iga_LSC->geometryX[(m*gnodesX*gnodesY + l*gnodesX+k)*dim+2] >= user.Lz-0.00001-user.spacing/2.0) ) {
          PetscInt index = (m*gnodesX*gnodesY + l*gnodesX+ k)*dof_LSC;
          PetscInt index_array[1]={0};
          index_array[0]=(m*gnodesX*gnodesY + l*gnodesX+ k)*dof_LSC;
          MatSetValuesLocal(A0_LSC,1,&index,1,index_array,bc_phi,INSERT_VALUES);CHKERRQ(ierr);
          MatSetValuesLocal(A0_LSC,1,index_array,1,&index,bc_phi,INSERT_VALUES);CHKERRQ(ierr);
          VecSetValueLocal(Res_LSC,index,0.0,INSERT_VALUES);CHKERRQ(ierr);
        }
      }
    }
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  ierr = probePressureSignals(&user, par);
   // Write Solution
