...
if(PETSC_FALSE){//Ptest > Pcr){ // We are not currently using this branch of the JWL
  ...
  *fprime   = ((R1*nu)/dens-omega*(1/dens+1/(R1*dens0)))*A*exp(-R1*nu) + ((R2*nu)/dens-omega*(1/dens+1/(R2*dens0)))*B*exp(-R2*nu) + omega*E0;
  *cs       = *fprime;
}
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      for(k=info.zs;k<info.zs+info.zm;k++){
        ...
        PetscReal phi = u_LS[k][j][i].phi + u_LSC[k][j][i].phi_prime;
...
PetscErrorCode OutputRestarts(PARAMETERS *par,Vec U,Vec V/*,ParticleManager &manager*/)
{
  ...
  // ##################################################
    ...
    // ##################################################
    convert << "RestartTime." << rank << "." << par->stepNumber << ".dat";
    fname = convert.str();
    ofstream Time;
    Time.open(fname, ios::out | ios::binary);
    Time.write( (char*)&par->currentTime, sizeof(double));
    Time.write( (char*)&par->timeStep,    sizeof(double));
    Time.write( (char*)&par->finalTime,   sizeof(double));
    Time.write( (char*)&par->Ynorm,       sizeof(double));
    Time.write( (char*)&par->Ynorm0,      sizeof(double));
		Time.close();
...
PetscErrorCode ReadLastResults(PARAMETERS *par,Vec U,Vec V,PetscInt StepRestart, PetscInt FreqRestarts/*,ParticleManager &manager*/,AppCtx *user)
{
  ...
  ierr = PetscViewerDestroy(&viewer);
    ...
    // Velocity
    ostringstream convert;
    convert << "RestartTime." << rank << "." << StepRestart << ".dat";
    string fname = convert.str();
    ifstream Time;
    Time.open(fname, ios::in | ios::binary);
    Time.read( (char*)&par->currentTime, sizeof(double));
    Time.read( (char*)&par->timeStep,    sizeof(double));
    Time.read( (char*)&par->finalTime,   sizeof(double));
    Time.read( (char*)&par->Ynorm,       sizeof(double));
    Time.read( (char*)&par->Ynorm0,      sizeof(double));
    Time.close();
  ...
  for(i=info.xs;i<info.xs+info.xm;i++){
    for(j=info.ys;j<info.ys+info.ym;j++){
      for(k=info.zs;k<info.zs+info.zm;k++){
        ...
        if(r <= R/* + factor/2.0*/ && z <= height /*+ h/2.0 && r < 1.5*R*/){
          u[k][j][i].alpha = 1.0;
          u[k][j][i].rho   = 1770.0;
        }
...
#undef __FUNCT__
#define __FUNCT__ "probePressureSignals11cm"
PetscErrorCode probePressureSignals11cm(AppCtx *user, PARAMETERS *par)
{
  PetscFunctionBegin;
  //PetscMPIInt rank;
  //MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
...
pt[0] = 0.11/(sqrt(3)*user->Lx);
pt[1] = 0.11/(sqrt(3)*user->Ly);
pt[2] = 0.11/(sqrt(3)*user->Lz);
...
sprintf(filenameResults,"./Output_ProbeData/pressureSignal11cm.csv");
...
#undef __FUNCT__
#define __FUNCT__ "probePressureSignals17cm"
PetscErrorCode probePressureSignals17cm(AppCtx *user, PARAMETERS *par)
{
  PetscFunctionBegin;
  //PetscMPIInt rank;
  //MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
...
pt[0] = 0.17/(sqrt(3)*user->Lx);
pt[1] = 0.17/(sqrt(3)*user->Ly);
pt[2] = 0.17/(sqrt(3)*user->Lz);
...
sprintf(filenameResults,"./Output_ProbeData/pressureSignal17cm.csv");
...
#undef __FUNCT__
#define __FUNCT__ "probePressureSignals27cm"
PetscErrorCode probePressureSignals27cm(AppCtx *user, PARAMETERS *par)
{
  PetscFunctionBegin;
  //PetscMPIInt rank;
  //MPI_Comm_rank(PETSC_COMM_WORLD,&rank);
...
pt[0] = 0.27/(sqrt(3)*user->Lx);
pt[1] = 0.27/(sqrt(3)*user->Ly);
pt[2] = 0.27/(sqrt(3)*user->Lz);
...
sprintf(filenameResults,"./Output_ProbeData/pressureSignal27cm.csv");
                        ...
                        void *ctx)
{
  ...
  /// Introduce a correction on alpha to preserve the physical bounds [0,1]
  // if(sqrt((alpha_1-1.0)*(alpha_1-1.0))<1.0e-2 || alpha_1 > 1.0){alpha_1 = 1.0;}
  // if(sqrt((alpha_1-0.0)*(alpha_1-0.0))<1.0e-2 || alpha_1 < 0.0){alpha_1 = 0.0;}
  PetscScalar alpha_2 = 1.0 - alpha_1;
...
F3[3] = P;
 ...
 tau_m = sqrt((Res[1]*Res[1]+Res[2]*Res[2]+Res[3]*Res[3])/(hu[1]));
 tau_t = sqrt(Res[4]*Res[4]/(hu[2]));
 tau_ls = sqrt(Res[5]*Res[5]/(hu[3]));

 // Upwinding cap
 if(DC*tau_c  > k_cap || tau_c  != tau_c  || isinf(tau_c)){tau_c = k_cap;}else{tau_c = DC*tau_c;}
 if(DC*tau_m  > k_cap || tau_m  != tau_m  || isinf(tau_m)){tau_m = k_cap;}else{tau_m = DC*tau_m;}
 if(DC*tau_t  > k_cap || tau_t  != tau_t  || isinf(tau_t)){tau_t = k_cap;}else{tau_t = DC*tau_t;}
 if(DC*tau_ls > k_cap || tau_ls != tau_ls || isinf(tau_ls)){tau_ls = k_cap;}else{tau_ls = DC*tau_ls;}
 ...
 for (a=0; a<nen; a++) {
   ...
   for (m=0;m<dim;m++){
     for (i=0;i<dof-2;i++){
       for (j=0;j<dof-2;j++){
               R[a][i+1] += N1[a][l]*K[l][m][i][j]*grad_u[j+1][m];
       }
     }
   }
   }
...
//Stabilization terms SUPG & DC
   ...
   for (i=0;i<dim;i++)
   {
               ...
               R[a][5] +=N1[a][i]*tau_ls*A0gradY[5][i];
   }
   }
  }
  return 0;
                         ...
                         AppCtx *user)
 {
     ...
     PetscScalar alpha_1_reference = 0.5*(tanh(phi0/localWidth)/0.987+1.0);
     ...
     for (a=0; a<nen; a++) {
       ...
       if(!pnt->atboundary){
         ...
         if(((1.0-tanh(phi_tilde/localWidth)*tanh(phi_tilde/localWidth))/localWidth)<1.0e-10){R[a][0] = 0.0;}
       }
     }
                       ...
                       PARAMETERS *par)
{
  ...
  for(int a = 0 ; a<nen ; a++){
    for(int b = 0 ; b<nen ; b++){
     ...
     if(((1.0-tanh(phi_tilde/h)*tanh(phi_tilde/h))/h)<1.0e-10){
        KK[a][0][a][0] = 1.0;
        if(a!=b){
          KK[a][0][b][0] = 0.0;}
        }
...
#undef __FUNCT__
#define __FUNCT__ "TSUpdateStage_GeneralizedAlpha_LS"
static PetscErrorCode TSUpdateStage_GeneralizedAlpha_LS(AppCtx *user,PARAMETERS *par,Vec dA)
{

  Vec            V1 = user->V1_LS, A1 = user->A1_LS;
  PetscReal      dt = par->timeStep/par->timeTildeFactor;
  PetscReal      Gamma   = user->Gamma;
  PetscErrorCode ierr;
  PetscFunctionBegin;
...
#undef __FUNCT__
#define __FUNCT__ "TSUpdateAlphaLevels_GeneralizedAlpha_LS"
static PetscErrorCode TSUpdateAlphaLevels_GeneralizedAlpha_LS(AppCtx *user)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;

  Vec            V1 = user->V1_LS, A1 = user->A1_LS, D1 = user->D1_LS;
  Vec            Va = user->Va_LS, Aa = user->Aa_LS, Da = user->Da_LS;
  Vec            V0 = user->V0_LS, A0 = user->A0_LS, D0 = user->D0_LS;
  PetscReal      Alpha_m = user->Alpha_m;
  PetscReal      Alpha_f = user->Alpha_f;
...
int main(int argc, char *argv[]) {
  ...
  par->initialTime  	   = 0.0;
  par->finalTime    	   = 15.0e-3;
  par->timeStep	         = 0.50e-7; //initial timestep very small
  ...
  user.enableSharpening  = PETSC_FALSE;

  user.spacing  = 0.0000;
  user.Lx       = 0.31+user.spacing;
  user.Ly       = 0.31+user.spacing;
  user.Lz       = 0.31+user.spacing;
  ...
  // Check the mass of explosive in the Initial Condition/ Restarted Condition //
  ...
  while(par->currentTime+par->timeStep <= par->finalTime+1e-9){
    ...
    if(par->currentTime > 0.25e-3 && par->timeStep < 5.0e-7){
      if(par->Ynorm <= 0.5*par->Ynorm0){
        par->timeStep *= 1.01;
      }
      if(par->Ynorm > 0.5*par->Ynorm0){
        par->timeStep *= 0.97;
    ...
    for (it=0; it<user.max_its ; it++) {
       ...
       PetscPrintf(PETSC_COMM_WORLD,":::::::::::::::::::::::::::::::::\n");
       // Check the mass of explosive after each pass  //
       ierr = IGASetFormIFunction(iga,Residual_massCalc,&user);CHKERRQ(ierr);
       ierr = IGAComputeIFunction(iga,0.0,user.A0,0.0,user.V0,Density);CHKERRQ(ierr);
       MPI_Barrier(PETSC_COMM_WORLD);
       double m1;
  ...
  if(it==0){par->Ynorm0 = ResNorm;}
  if(it==2){par->Ynorm  = ResNorm;}
  ////////////////////////////
...
ierr = InitializeLS(iga,iga_LS,t,user.V0,user.V0_LS,&user,par);CHKERRQ(ierr);
...
PetscReal t_LS = 0.0;
par->timeTildeFactor = 0.0005;
PetscReal dt_LS = par->timeStep/par->timeTildeFactor;
double ResNormLS = 0.0;
PetscReal Alpha_f = user.Alpha_f;
PetscReal Alpha_m = user.Alpha_m;
double ResNormRef = 1.0;
 ...
 for(int II = 0 ; II < 5 ; II++){
   ...
   for(int RD_IT = 0 ; RD_IT < 3 ; RD_IT++){
    ...
    ierr = TSUpdateStage_GeneralizedAlpha_LS(&user,par,user.dA_LS);CHKERRQ(ierr);
...
//##################################################################################################
 ...
 ierr = UpdatePhi(iga,iga_LS,iga_LSC,user.V1,user.V1_LS,user.V0_LSC,&user,par);CHKERRQ(ierr);
 MPI_Barrier(PETSC_COMM_WORLD);

 double m_1;
 ierr = IGASetFormIFunction(iga,Residual_massCalc,&user);CHKERRQ(ierr);
 ierr = IGAComputeIFunction(iga,1.0,user.A0,1.0,user.V1,Density);CHKERRQ(ierr);

 VecNorm(Density,NORM_1,&m_1);
 ierr = IGASetFormIFunction(iga,Residual,&user);CHKERRQ(ierr);
 PetscPrintf(PETSC_COMM_WORLD,"Total Explosive Mass Before correction = %e\n", m_1*1.0e6);
 ...
 ierr = UpdatePhi(iga,iga_LS,iga_LSC,user.V0,user.V0_LS,user.V0_LSC,&user,par);CHKERRQ(ierr);
 MPI_Barrier(PETSC_COMM_WORLD);
 ...
 double m1;
 VecNorm(Density,NORM_1,&m1);
 PetscPrintf(PETSC_COMM_WORLD,"Mass After Corrections = %e [mg]\n", m1*1.0e6);
 // // ///////////////////////////////////////////////////// //
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Sync V0 and V1 before writing restart files
  ...
  ierr = probePressureSignals11cm(&user, par);
  ierr = probePressureSignals17cm(&user, par);
  ierr = probePressureSignals27cm(&user, par);
  ...
  par->stepNumber++;
   ...
   if (par->stepNumber % par->FreqResults == 0) {
    ...
    if (par->stepNumber % user.FreqRestarts == 0) {
        ...
        MPI_Barrier(PETSC_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
        PetscPrintf(PETSC_COMM_WORLD, "Restarts Written at time %e step %d. Contintuing...\n", par->currentTime, par->stepNumber);
    }
